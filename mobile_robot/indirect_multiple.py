""" Script that solves the optimal control problem for the Mobile Robot problem using indirect multiple shooting
...
def multiple_shooting(initial_states, final_states, guesses, nx, K, states_str, nu, control_str, ptot_guess, is_print=False):
    ...
    ptot0guess = ptot_guess*np.ones([2*nx, K-1])
    ...
    obj_sol = root(objective, changing, method="hybr", tol=1e-5)
...
def main():
    ...
    multiple_shooting(initial_states, final_states, guesses,
                      nx, K, states_str, nu, control_str, ptot_guess, False)
